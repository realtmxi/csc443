 ...
 *
 * It will merge the leaves into a leaf file and the internal nodes into an internal node file.
 * After merging is complete, the leaf file will be appended to the end of the internal node file.
 * This maintains the structure of the BTree SST for a complexity of O(NlogN + N) where N is the
 * number of pages in the BTree.
 */
class LSMTree {
 public:
  explicit LSMTree();
  ...
  // Tombstones will remove the key from the page.
  // The overflow page will contain the keys that could not fit in the new page and will be used in the next merge.
  void MergePages(BTreePage *page1, BTreePage *page2, BTreePage *output_page, BTreePage *overflow_page);
