...
int AVLTree::get(AVLNode *node, int key) {
  if (!node)
    return -1;
...
void AVLTree::put(int key, int value) { root = insert(root, key, value); }

int AVLTree::get(int key) { return get(root, key); }

// Scan function to get key-value pairs in sorted order (added)
std::vector<std::pair<int, int>> AVLTree::scan() {
  std::vector<std::pair<int, int>> result;
